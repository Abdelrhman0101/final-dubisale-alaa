// lib/presentation/screens/other_services_ad_screen.dart
...
class _OtherServicesAdScreenState extends State<OtherServicesAdScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (!mounted) return;
      // جلب بيانات النماذج بدون أي اعتماد على التوكن
  ...
  Future<void> _getCurrentLocation() async {
    ...
    try {
      ...
      if (mapsProvider.currentLocationData != null) {
        ...
        if(mounted) setState(() { selectedLatLng = latLng; if (address != null) selectedLocation = address;});
      }
    } catch (e) {
      if(mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to get location: $e'), backgroundColor: Colors.red));
    } finally {
  ...
  Future<void> _navigateToLocationPicker() async {
    final result = await context.push('/location_picker', extra: {'initialLatLng': selectedLatLng});
      if (result != null && result is Map<String, dynamic>) {
        ...
        if(location != null) setState(() { selectedLatLng = location; if(address != null) selectedLocation = address;});
      }
  }
  ...
  Future<void> _validateAndProceedToNext() async {
    ...
    final infoProvider = context.read<OtherServicesInfoProvider>();
    final adData = {
      'adType': 'other_service',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Consumer<OtherServicesInfoProvider>(
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 7.h),
                    Center(child: Text(s.otherServicesAds, style: TextStyle(fontWeight: FontWeight.w500, fontSize: 24.sp, color: KTextColor))),
                    SizedBox(height: 10.h),
                    _buildFormRow([
                      _buildSingleSelectField(context, s.emirate, selectedEmirate, infoProvider.emirateDisplayNames, (selection) => setState(() { selectedEmirate = selection; selectedDistrict = null; }), isRequired: true),
                    ...
                    _buildFormRow([
                       _buildTitledTextFormField(s.area, _areaController, borderColor, currentLocale, hintText: 'Jumeira1', isRequired: true),
                       _buildTitledTextFormField(s.price, _priceController, borderColor, currentLocale, hintText: '1000', isNumber: true, isRequired: true),
                    ]),
                    const SizedBox(height: 7),
                     _buildFormRow([
                       _buildTitledTextFormField(s.serviceName, _serviceNameController, borderColor, currentLocale, hintText: 'Account Audit', isRequired: true),
                    ...
                    const SizedBox(height: 7),
                    _buildTitledTextFormField(s.title, _titleController, borderColor, currentLocale, minLines: 2, isRequired: true, maxLength: 93),
                    const SizedBox(height: 7),
                    ...
                    const SizedBox(height: 7),
                    _buildFormRow([
                       TitledSelectOrAddField(title: s.phoneNumber, value: selectedPhoneNumber, items: infoProvider.phoneNumbers, onChanged: (newValue) => setState(() => selectedPhoneNumber = newValue), isNumeric: true),
                       TitledSelectOrAddField(title: s.whatsApp, value: selectedWhatsAppNumber, items: infoProvider.whatsappNumbers, onChanged: (newValue) => setState(() => selectedWhatsAppNumber = newValue), isNumeric: true),
                    ]),
                    const SizedBox(height: 7),
                    ...
                    if(_mainImageBytes != null)
                        Padding(padding: const EdgeInsets.symmetric(vertical: 8), child: Center(child: Image.memory(_mainImageBytes!, height: 150, fit: BoxFit.cover))),
                    const SizedBox(height: 7),
                    Text(s.location, style: TextStyle(fontWeight: FontWeight.w600, fontSize: 16.sp, color: KTextColor)),
                    SizedBox(height: 4.h),
                    Directionality(textDirection: TextDirection.ltr, child: Row(children: [ SvgPicture.asset('assets/icons/locationicon.svg', width: 20.w, height: 20.h), SizedBox(width: 8.w), Expanded(child: Text(selectedLocation.isNotEmpty ? selectedLocation : 'No location selected', style: TextStyle(fontSize: 14.sp, color: KTextColor, fontWeight: FontWeight.w500)))])),
                     SizedBox(height: 8.h),
                    _buildMapSection(context),
                    const SizedBox(height: 12),
                    SizedBox(width: double.infinity, child: ElevatedButton(onPressed: _validateAndProceedToNext, child: Text(s.next, style: TextStyle(fontSize: 16.sp, fontWeight: FontWeight.bold, color: Colors.white)), style: ElevatedButton.styleFrom(backgroundColor: KPrimaryColor, padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))))),
...
class TitledDescriptionBoxSimple extends StatelessWidget { final String title; final TextEditingController controller; final Color borderColor; const TitledDescriptionBoxSimple({Key? key, required this.title, required this.controller, required this.borderColor}) : super(key: key); @override Widget build(BuildContext context) { return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: TextStyle(fontWeight: FontWeight.w600, color: KTextColor, fontSize: 14.sp)), const SizedBox(height: 4), Container(decoration: BoxDecoration(borderRadius: BorderRadius.circular(8), border: Border.all(color: borderColor)), child: TextFormField(controller: controller, maxLines: 5, minLines: 3, maxLength: 5000, style: TextStyle(fontWeight: FontWeight.w500, color: KTextColor, fontSize: 14.sp), decoration: const InputDecoration(border: InputBorder.none, contentPadding: EdgeInsets.all(12), counterText: "")))]);} }