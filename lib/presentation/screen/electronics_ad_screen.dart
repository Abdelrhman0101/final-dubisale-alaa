...
class ElectronicsAdScreen extends StatefulWidget {
  final Function(Locale) onLanguageChange;
  const ElectronicsAdScreen({Key? key, required this.onLanguageChange}) : super(key: key);
  @override
  State<ElectronicsAdScreen> createState() => _ElectronicsAdScreenState();
}
...
class _ElectronicsAdScreenState extends State<ElectronicsAdScreen> {
  ...
  Future<void> _pickThumbnailImages() async {
    ...
    if (pickedImages.isNotEmpty) {
        if(pickedImages.length > remainingSlots){
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('You can only add $remainingSlots more images.')));
            setState(() => _thumbnailImages.addAll(pickedImages.sublist(0, remainingSlots).map((e) => File(e.path))));
        } else {
            setState(() => _thumbnailImages.addAll(pickedImages.map((e) => File(e.path))));
        }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: Colors.white,
      body: Consumer<ElectronicsInfoProvider>(
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 7.h),
                    Center(child: Text(s.electronicsAndHomeAppliancesAds, textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.w500, fontSize: 24.sp, color: KTextColor))),
                    ...
                    _buildFormRow([
                      _buildTitledTextFormField(s.area, _areaController, borderColor, currentLocale, hintText: 'Souq Gawa', isRequired: true),
                      _buildTitledTextFormField(s.price, _priceController, borderColor, currentLocale, hintText: '3500', isNumber: true, isRequired: true),
                    ]),
                    const SizedBox(height: 7),
                    _buildFormRow([
                    ...
                    TitledSelectOrAddField(
                      ...
                      onAddNew: (value) async { /* ... Logic ... */ }
                    ),
                    const SizedBox(height: 7),
                    _buildFormRow([
                      TitledSelectOrAddField(title: s.phoneNumber, value: selectedPhoneNumber, items: infoProvider.phoneNumbers, onChanged: (newValue) => setState(() => selectedPhoneNumber = newValue), isNumeric: true, onAddNew: (value) async { /* ... Logic ... */ }),
                      TitledSelectOrAddField(title: s.whatsApp, value: selectedWhatsAppNumber, items: infoProvider.whatsappNumbers, onChanged: (newValue) => setState(() => selectedWhatsAppNumber = newValue), isNumeric: true, onAddNew: (value) async { /* ... Logic ... */ }),
                    ]),
                    const SizedBox(height: 7),
                    ...
                    if (_mainImage != null) Padding(padding: const EdgeInsets.symmetric(vertical: 8), child: Center(child: ClipRRect(borderRadius: BorderRadius.circular(8), child: Image.file(_mainImage!, height: 150, fit: BoxFit.cover)))),
                    const SizedBox(height: 7),
                    _buildImageButton('${s.add4Images} (${_thumbnailImages.length}/4)', Icons.add_photo_alternate_outlined, borderColor, onPressed: _pickThumbnailImages),
                    if (_thumbnailImages.isNotEmpty) Padding(padding: const EdgeInsets.symmetric(vertical: 8.0), child: Wrap(spacing: 8, runSpacing: 8, children: _thumbnailImages.asMap().entries.map((entry) { int idx = entry.key; File img = entry.value; return Stack(children: [ClipRRect(borderRadius: BorderRadius.circular(8), child: Image.file(img, width: 80, height: 80, fit: BoxFit.cover)), Positioned(top: 2, right: 2, child: GestureDetector(onTap: () => _removeThumbnailImage(idx), child: Container(decoration: BoxDecoration(color: Colors.black.withOpacity(0.6), shape: BoxShape.circle), child: const Icon(Icons.close, color: Colors.white, size: 16))))]);}).toList())),
                    const SizedBox(height: 7),
