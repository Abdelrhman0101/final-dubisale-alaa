// lib/presentation/screens/other_service_offer_box.dart
...
// تم إزالة الاعتماد على FlutterSecureStorage لأن الصفحة لا يجب أن تعتمد على التوكن
...
class _OtherServiceOfferBoxState extends State<OtherServiceOfferBox> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (!mounted) return;
      // جلب البيانات بدون أي اعتماد على التوكن
  ...
  void _applyFilters() {
    ...
    if (_priceFrom != null && _priceFrom!.isNotEmpty) filters['price_from'] = _priceFrom!;
    if (_priceTo != null && _priceTo!.isNotEmpty) filters['price_to'] = _priceTo!;
    if (_selectedSections.isNotEmpty) filters['section_type'] = _selectedSections.join(',');
    if (_selectedServices.isNotEmpty) filters['service_name'] = _selectedServices.join(',');
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        body: SafeArea(
          child: Consumer2<OtherServicesAdProvider, OtherServicesInfoProvider>(
              ...
              return SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    GestureDetector(onTap: () => context.pop(), child: Row(children: [const SizedBox(width: 18), Icon(Icons.arrow_back_ios, color: KTextColor, size: 17.sp), Transform.translate(offset: Offset(-3.w, 0), child: Text(s.back, style: TextStyle(fontSize: 14.sp, fontWeight: FontWeight.w500, color: KTextColor)))])),
                    SizedBox(height: 7.h),
                    Center(child: Text("Offers Box", style: TextStyle(fontWeight: FontWeight.w500, fontSize: 24.sp, color: KTextColor))),
                    SizedBox(height: 10.h),
                    Padding(
                      ...
                      child: Container(height: 35.h,
                        child: Row(
                          ...
                          children: [
                            ...
                            Expanded(child: Row(
                                children: [
                                  Expanded(child: _buildMultiSelectField(context, s.section, _selectedSections, infoProvider.sectionTypes,
                                      (selection) {
                                        setState(() => _selectedSections = selection);
                                        _applyFilters();
                                      }, isFilter: true, isLoading: infoProvider.isLoading)),
                                  SizedBox(width: 3.w),
                                  Expanded(child: _buildMultiSelectField(context, s.service, _selectedServices, adProvider.serviceNames,
                                      (selection) {
                                        setState(() => _selectedServices = selection);
                                        _applyFilters();
                                      }, isFilter: true, isLoading: adProvider.isLoading)),
                                  SizedBox(width: 3.w),
                                  Expanded(child: _buildRangePickerField(context, title: s.price, fromValue: _priceFrom, toValue: _priceTo, unit: "AED", isFilter: true, onTap: () async {
                                      final result = await _showRangePicker(context, title: s.price, initialFrom: _priceFrom, initialTo: _priceTo, unit: "AED");
                                      if (result != null) {
                                        setState(() {
                                          _priceFrom = result['from'] == "" ? null : result['from'];
                                          _priceTo = result['to'] == "" ? null : result['to'];
                                        });
                                        final adProvider = context.read<OtherServicesAdProvider>();
                                        if ((_priceFrom == null || _priceFrom!.isEmpty) &&
                                            (_priceTo == null || _priceTo!.isEmpty)) {
                                          adProvider.clearOfferPriceFilters();
                                        } else {
                                          adProvider.setOfferPriceFilters(_priceFrom, _priceTo);
                                        }
                                        _applyFilters();
                                      }
                                    })),
                    ...
                    SizedBox(height: 6.h),
                    Padding(padding: EdgeInsetsDirectional.symmetric(horizontal: 8.w), child: LayoutBuilder(builder: (context, constraints) { bool isSmallScreen = MediaQuery.of(context).size.width <= 370; return Row(children: [Text('${s.ad} ${adProvider.offerAds.length}', style: TextStyle(fontSize: 12.sp, color: KTextColor, fontWeight: FontWeight.w400)), SizedBox(width: isSmallScreen ? 40.w : 35.w), Expanded(child: Container(height: 37.h, padding: EdgeInsetsDirectional.symmetric(horizontal: 8.w), decoration: BoxDecoration(border: Border.all(color: const Color(0xFF08C2C9)), borderRadius: BorderRadius.circular(8.r)), child: Row(children: [SvgPicture.asset('assets/icons/locationicon.svg', width: 18.w, height: 18.h), SizedBox(width: 12.w), Expanded(child: Text(s.sort, overflow: TextOverflow.ellipsis, style: TextStyle(fontWeight: FontWeight.w600, color: KTextColor, fontSize: 12.sp))), SizedBox(width: 1.w), SizedBox(width: 35.w, child: Transform.scale(scale: 0.8, child: Switch(value: true, onChanged: (val) {}, activeColor: Colors.white, activeTrackColor: const Color.fromRGBO(8, 194, 201, 1), inactiveThumbColor: Colors.white, inactiveTrackColor: Colors.grey[300])))])))]);})),
                    SizedBox(height: 5.h),
                    ...
                    if (adProvider.isLoadingOffers)
                      ...
                      Center(heightFactor: 5, child: Text(s.noResultsFound, style: TextStyle(color: KTextColor)))
                    else
                      Padding(
                        ...
                        child: GridView.builder(
                          ...
                          gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, mainAxisSpacing: 6, childAspectRatio: .89),
                          itemBuilder: (context, index) {
                            final ad = adProvider.offerAds[index];
                            return GestureDetector(
                              onTap: () => context.push('/other_service-details/${ad.id}'),
                              child: Padding(
                                      ...
                                      Expanded(
                                        child: Padding(
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              Text("${NumberFormatter.formatPrice(ad.price)}", style: TextStyle(color: Colors.red, fontWeight: FontWeight.w600, fontSize: 12.sp)),
                                              Text(ad.serviceName!, style: TextStyle(fontWeight: FontWeight.w600, fontSize: 12.sp, color: KTextColor), maxLines: 1, overflow: TextOverflow.ellipsis),
                                              Text(ad.advertiserName, style: TextStyle(fontWeight: FontWeight.w600, fontSize: 12.sp, color: KTextColor)),
                                              Row(children: [ SvgPicture.asset( 'assets/icons/Vector.svg', width: 10.5.w, height: 13.5.h), const SizedBox(width: 5), Expanded(child: Text("${ad.emirate} ${ad.district}", style: TextStyle(fontSize: 12.sp, color: const Color.fromRGBO(0, 30, 91, .75), fontWeight: FontWeight.w600), overflow: TextOverflow.ellipsis))]),
                                            ],
                                          ),
                                        ),
                                      ),
...
Size getCardSize(double screenWidth) {
  if (screenWidth <= 320) { return const Size(120, 140); } 
  else if (screenWidth <= 375) { return const Size(135, 150); } 
  else if (screenWidth <= 430) { return const Size(150, 160); } 
  else { return const Size(165, 175); }
}
      ...
      children: [
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: Text(
              displayText,
              style: TextStyle(fontWeight: FontWeight.w500, color: (selectedValues.isEmpty && !isLoading) ? KTextColor : KTextColor, fontSize: 9.5),
